...
class WaitingRoomConsumer(AsyncWebsocketConsumer):
    """
    WebSocket consumer for real-time waiting room queue updates.
    Broadcasts state changes to all connected doctors.
    """
    
    async def connect(self):
        """Called when a WebSocket connection is established."""
        self.waiting_room_group = "waiting_room"
        ...
        await self.accept()
        print(f"✓ Doctor {user.id} joined waiting room queue")
    ...
    async def queue_update(self, event):
        """
        Broadcoast a queue update to the client.
        The event contains the updated status and appointment info.
        """
        await self.send(text_data=json.dumps({
            'type': 'queue_update',
