...
class Appointment(models.Model):
    ...
    TYPE_CHOICES = (
        ...
        ("imaging", "Imaging Session"),
        ("lab", "Lab Test / Sample Collection"),
        ("follow-up", "Follow-up Appointment"),
    )

    MODALITY_CHOICES = (
        ("MR", "MRI"), ("CT", "CT Scan"), ("XR", "X-Ray"), ("US", "Ultrasound"),
        ("PET", "PET Scan"), ("NM", "Nuclear Medicine"), ("DXA", "Bone Density"),
        ("MG", "Mammography"), ("RF", "Fluoroscopy"), ("OT", "Other"),
    )
    ...
    appointment_type = models.CharField(max_length=20, choices=TYPE_CHOICES, default="doctor")
    parent_appointment = models.ForeignKey('self', on_delete=models.SET_NULL, null=True, blank=True, 
                                           related_name="follow_ups")
    
    imaging_modality = models.CharField(max_length=10, choices=MODALITY_CHOICES, null=True, blank=True)
    ...
    location_room = models.CharField(max_length=50, blank=True, default="Room 101")
    notes = models.TextField(blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def clean(self):
        ...
        # 1. Conflict Detection: Check for overlapping appointments for the SAME doctor
        end_time = self.scheduled_at + timedelta(minutes=self.duration_minutes)
        all_doctor_apts = Appointment.objects.filter(doctor=self.doctor).exclude(pk=self.pk)
        for apt in all_doctor_apts:
            apt_end = apt.scheduled_at + timedelta(minutes=apt.duration_minutes)
            if self.scheduled_at < apt_end and end_time > apt.scheduled_at:
                raise ValidationError(f"Conflict: Dr. {self.doctor.get_full_name()} already has an appointment ('{apt}') at this time.")
        

        # 2. Availability Check: Check if within doctor's working hours
        day_of_week = self.scheduled_at.weekday()
        appt_time = self.scheduled_at.time()
        ...
        if not availabilities.exists():
            raise ValidationError(f"Dr. {self.doctor.get_full_name()} is not available on {self.scheduled_at.strftime('%A')}.")
        
        is_available = False
        appt_end_time = (self.scheduled_at + timedelta(minutes=self.duration_minutes)).time()
        for avail in availabilities:
            # Check if appointment start and end fall within this availability slot
            if appt_time >= avail.start_time and appt_end_time <= avail.end_time:
                is_available = True
                break
        
        if not is_available:
            raise ValidationError(f"Appointment time {appt_time} is outside Dr. {self.doctor.get_full_name()}'s working hours.")
