...
import { ArrowLeft, Activity, Image as ImageIcon, Box, Layers, Info, Sun, Search, Move, RotateCcw, BrainCircuit, FlaskConical } from 'lucide-react';
...
const DicomPage = () => {
    ...
    const [maskCache, setMaskCache] = useState({}); // { [seriesId]: { data: Uint8Array, shape: [D, H, W], base64: string } }
        ...
        <div className="h-screen w-full bg-black flex overflow-hidden font-sans text-slate-300 select-none">
            ...
            <aside className="w-16 bg-[#070708] border-r border-white/5 flex flex-col items-center py-8 gap-6 z-20">
                ...
                <button 
                    ...
                    className="p-3 text-cyan-400 bg-cyan-600/10 hover:bg-cyan-500 hover:text-white border border-cyan-500/20 hover:border-cyan-400 rounded-xl transition-all relative group shadow-inner"
                >
                    <FlaskConical size={22} className="group-hover:rotate-12 transition-transform" />
                    <span className="absolute left-full ml-4 px-3 py-1.5 bg-cyan-900 border border-cyan-500/30 text-white font-bold tracking-widest uppercase text-[10px] rounded shadow-xl opacity-0 group-hover:opacity-100 z-50 whitespace-nowrap pointer-events-none">
                        Open Lab Pro
                    </span>
                </button>
            ...
            <main className="flex-1 relative bg-[#050507] flex items-center justify-center overflow-hidden">
                {loading ? (
                    ...
                    >
                        ...
                        {/* AI Clinical Index Overlay */}
                        {maskCache[currentSeriesId] && !isSegmenting && (
                            <div className="absolute bottom-10 left-10 p-5 bg-[#070708]/90 border border-emerald-500/30 rounded-2xl backdrop-blur-xl shadow-2xl flex flex-col gap-3 min-w-[280px] z-50 transition-all duration-300 hover:scale-105 border-l-4 border-l-emerald-500">
                                <div className="flex items-center justify-between border-b border-white/10 pb-3">
                                    <div className="flex items-center gap-2">
                                        <BrainCircuit className="text-emerald-500 animate-pulse" size={20} />
                                        <h3 className="text-xs font-black text-white tracking-widest uppercase">AI Clinical Index</h3>
                                    </div>
                                    <span className="text-[10px] text-white/30 font-mono italic">v3.0.4-turbo</span>
                                </div>
                                
                                <div className="grid grid-cols-2 gap-y-3 gap-x-4">
                                    <div className="flex flex-col">
                                        <span className="text-[10px] text-white/40 uppercase tracking-tighter">Confidence</span>
                                        <span className="text-sm font-bold text-emerald-400 font-mono">
                                            {maskCache[currentSeriesId].confidence ? `${maskCache[currentSeriesId].confidence.toFixed(1)}%` : 'N/A'}
                                        </span>
                                    </div>
                                    <div className="flex flex-col text-right">
                                        <span className="text-[10px] text-white/40 uppercase tracking-tighter">Volume</span>
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-[10px] text-white/40 uppercase tracking-tighter">Avg IoU</span>
                                        <span className="text-sm font-bold text-purple-400 font-mono">
                                            {maskCache[currentSeriesId].iou ? maskCache[currentSeriesId].iou.toFixed(3) : 'N/A'}
                                        </span>
                                    </div>
                                    <div className="flex flex-col text-right">
                                        <span className="text-[10px] text-white/40 uppercase tracking-tighter">Precision</span>
                                        <span className={`text-sm font-bold font-mono ${maskCache[currentSeriesId].confidence > 85 ? 'text-emerald-400' : 'text-amber-400'}`}>
                                            {maskCache[currentSeriesId].confidence && maskCache[currentSeriesId].confidence > 85 ? 'HIGH' : 'MED'}
                                        </span>
                                    </div>
                                </div>
                                
                                <div className="mt-1 text-[8px] text-white/20 font-mono uppercase tracking-widest flex justify-between">
                                    <span>Spatial Sync Optimized</span>
                                    <span>TURBO-PRECISION</span>
                                </div>
                                
                                <button 
                                    onClick={() => setIsLabOpen(true)}
                                    className="mt-2 w-full flex items-center justify-center gap-2 p-3 bg-emerald-600/10 hover:bg-emerald-600/20 border border-emerald-500/20 text-emerald-400 font-bold uppercase tracking-widest text-[10px] rounded-xl transition-all shadow-inner"
                                >
                                    <FlaskConical size={14} /> EXTRACT TO LAB PRO
                                </button>
                            </div>
                        )}
                    </div>
            ...
            {/* AI Lab Station Overlay */}
            {isLabOpen && (
                <div className="fixed inset-0 z-[3000]">
                    <LabViewer
                        studyId={studyId}
                        seriesId={currentSeriesId}
                        maskBase64={maskCache[currentSeriesId]?.base64 || null}
                        maskShape={maskCache[currentSeriesId]?.shape || null}
